...
for iteration in range(1):
    ...
    for key in sorted(dict_conservative_features):
        ...
        y_pred = np.round(model.predict(np.stack(dict_conservative_features[key])), 0)
    ...
    for key in sorted(dict_liberal_features):
        ...
        y_pred = np.round(model.predict(np.stack(dict_liberal_features[key])), 0)
...
plt.savefig("../results/effectiveness/adult/bank_note_binned_trust_wdt_tensor.png")
...
plt.savefig("../results/effectiveness/adult/bank_note_binned_trust_sdt_tensor.png")
